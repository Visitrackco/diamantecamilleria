...
@Component({
  ...
  styleUrls: ['./programadas.page.scss'],
})
export class ProgramadasPage implements OnInit {
  ...
  constructor(
    ...
    // Actividades Programadas: no se reciben solicitudes nuevas por socket (no es necesario que aparezcan aqui).
  ...
  async midata() {
    ...
    if (login) {
      ...
      this.isCentral = login[0].isCentral == 1;
      ...
      this.api.getDate({
        ...
        this.api.getLocationByZone({
          ...
          this.api.getWMotivos(login[0].WorkZone, login[0].token, false
          ).then((rsMotivo) => {
  ...
  async getSolicitudes() {
    ...
    if (login) {
      ...
      this.api.apiPost('searchActivity3', {
        token: login[0].token,
        WorkZoneID: login[0].WorkZone,
        Format: 'America/Bogota',
  ...
  async reloadMotivos() {
    const login = await this.stg.getLogin();
    if (login) {
      const rsMotivo = await this.api.getWMotivos(login[0].WorkZone, login[0].token, false);
      if (rsMotivo) {
        this.isCOH = false;
        this.perfilName = '';
        this.perfilIsCOH = false;
